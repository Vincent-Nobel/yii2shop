 ...
 */
class m170803_113115_create_order_table extends Migration
{
    /**
     * @inheritdoc
     */
    public function up()
    {
        $this->createTable('order', [
            'id' => $this->primaryKey(),
            //member_id	int	用户id
            'member_id'=>$this->string(50)->comment('用户id'),
//name	varchar(50)	收货人
...
//delivery_name	varchar	配送方式名称
            'delivery_name'=>$this->string(100)->comment('配送方式名称'),
//delivery_price	float	配送方式价格
            'delivery_price'=>$this->float(6,2)->comment('配送方式价格'),
//payment_id	int	支付方式id
            'payment_id'=>$this->integer()->comment('支付方式id'),
//payment_name	varchar	支付方式名称
            'payment_name'=>$this->string(100)->comment('支付方式名称'),
//total	decimal	订单金额
