...
//                var_dump($mode);exit;
                ?>
                <dl>
                    <dt><?=$mode->id?>.<?=$mode->username?>&nbsp;
                        <?=\frontend\models\Locations::findOne(['id'=>$mode->province])->name?>&emsp;
                        <?=\frontend\models\Locations::findOne(['id'=>$mode->city])->name?>&emsp;
                        <?=\frontend\models\Locations::findOne(['id'=>$mode->area])->name?>&emsp;
                        <?=$mode->address?>&emsp;
                        <?=$mode->cell?>
                    </dt>
                    <dd>
                        <?=\yii\bootstrap\Html::a('编辑',['member/edit','id'=>$mode->id])?>
                        &emsp;
                        <?=\yii\bootstrap\Html::a('删除',['member/del','id'=>$mode->id])?>
                    </dd>
                </dl>
                <?php
            endforeach;
        ...
        <div class="address_bd mt10">
            ...
            <?php
            \yii\widgets\ActiveForm::begin(['id'=>'address_form']);
            ?>
            <ul>
                <li>
                    <label for=""><span>*</span>收 货 人：</label>
                    <input type="text" name="Address[username]" class="txt" value="<?=$model->username?>" />
                </li>
                <!--  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
                <li id="content">
                ...
                <li>
                    ...
                    <input type="text" name="Address[cell]" class="txt" value="<?=$model->cell?>" />
                </li>
                <!--                    <li>-->
                <!--                        <label for="">&nbsp;</label>-->
                ...
                <li>
                    <label for="">&nbsp;</label>
                    <input type="button" name="" class="login_btn" value="保存" />
                </li>
            </ul>
            <!--            </form>-->
            <?php
    ...
    $(function(){
        ...
        $.getJSON(url,args,function(data){
            ...
            $(data).each(function(i,v){
                //var seleted= (v.id==<?=$model->province?>)?"selected":''
        ...
        $('#province').change(function(){
            ...
            $.getJSON(url,args,function(data){
                ...
                $(data).each(function(i,v){
                    //获取到的数据展示在#city中
                    // var seleted= (v.id==<?=$model->city?>)?"selected":''
                    html += '<option  value="'+v.id+'">'+v.name+'</option>'
                    //$(html).appendTo($('#city'));
                });
                $('#city').html(html)
